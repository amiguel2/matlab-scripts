...
% [temp, timestamps] = xlsread(file, 'Sheet1');
...
% data
[num1,txt1,raw1] = xlsread(file,'B40:FI135'); % label 1 (OD)
...
blank = num1(:,15);
% blank = xlsread('2017-03-18 MG FACS MreB Library A22 0p5 10000 dilution.xlsx','B54:FH54')
% blank = blank';
blankwells = [15];

% t = zeros(1,size(temp,1));
% count = 1;
t = xlsread(file,'B38:FI38');
t = t/60;
% for j = 1:size(temp,1)
%     t(count) = temp(j,3)*60 + temp(j,4) + temp(j,5)/60;
%     if count >180
%         t(count)= t(count)
%     end
%     count = count + 1;
...
% plots
od = num1-repmat(blank,[1,wellformat]);
logod = od(1:end,:);
for i = 1:wellformat
