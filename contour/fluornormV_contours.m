function fluornormV_contours(varargin)
...
directory = pwd;
prefix = '';
suffix = '_Phase_07-Jun-2018_CONTOURS_FILT';
...
for i = 1:numel(list)
    ...
    if isfield(f,'frame')
        ...
        fprintf('Calculating average fluorescence (volume normalized): %s\n',list(i).name);
        for k = 1:numel(f.frame)
            fprintf('%d ',k)
            for j = 1:numel(f.frame(k).object)
                %                     fprintf('%d\t',j);
                if isfield(f.frame(k).object(j),'fluor_internal_profile')
                    mesh = f.frame(k).object(j).mesh;
                    % volume
                    if isfield(f.frame(k).object(j),'volume')
                        V = f.frame(k).object(j).volume;
                    else
                    ...
                    % fluor
                    totfluor = sum(f.frame(k).object(j).fluor_internal_profile - blank);
                    f.frame(k).object(j).blanked_totfluor = totfluor;
                    if ~isempty(V)
                    f.frame(k).object(j).fluornormV = totfluor/V;
                    count_file = count_file + 1;
                    else
                        f.frame(k).object(j).fluornormV = [];
                    end
                else
                    f.frame(k).object(j).fluornormV = [];
                end
