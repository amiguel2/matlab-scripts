function convert_features_to_stacks_nooutput(varargin)
tic
...
if make_one_stack
    if contains(phasechannel,'feature')
        ...
        for j=1:numel(d)
            if d(j).isdir
                reorderfiles([folder d(j).name '/'],phasechannel)
            end
        end
   ...
   if stack_gfp
       if contains(gfpchannel,'feature')
           ...
           for j=1:numel(d)
               if d(j).isdir
                   reorderfiles([folder d(j).name '/'],gfpchannel)
               end
           end
           filelist2 = rdir([folder dirprefix '*/' gfpchannel '*.tif']);
           concat_tiffs(filelist1,[o_folder phasestackname])
       else
