...
function printdata(legname,data,dataerror,data_name)
    % header prep
    fprintf('Mean Values\n')
    fprintf('SampleName\t'); fprintf('%s\t',data_name{:});
    
    % print samples mean value
    for i = 1:numel(legname)
           fprintf('\n%s\t',legname{i}); % sample name
           fprintf('%f\t',data(i,:)); % mean values 
    end
    
    % header prep
    fprintf('\nError Values')
    fprintf('\nSampleName\t'); fprintf('%s\t',data_name{:});
    
    % print samples error value
    for i = 1:numel(legname)
           fprintf('\n%s\t',legname{i}); % sample name
           fprintf('%f\t',dataerror(i,:)) % mean values
    end
