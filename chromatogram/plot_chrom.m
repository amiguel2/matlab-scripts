 ...
 function plot_labels(sample,shift)
    for peak_idx = 1:numel(sample.Peaks)
        ...
        if mod(peak_idx,2) == 0
            ...
            text(sample.chrom.t(pk_tip),sample.chrom.final(pk_tip)+shift,sample.Peaks(peak_idx).name,'Color',[0 0 0])
        else
            rectangle('Position',[x,y,w,h],'FaceColor',[0 0 1],'EdgeColor',[0 0 1]);
            text(sample.chrom.t(pk_tip),sample.chrom.final(pk_tip)+shift,sample.Peaks(peak_idx).name,'Color',[0 0 1])
        end
